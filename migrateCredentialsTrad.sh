...
--user "$CONTROLLER_SOURCE_USER_TOKEN" "${CONTROLLER_SOURCE_URL}/scriptText" -o "$GEN_DIR/tmp-folder.txt"
...
curl -v --data-urlencode "script=$(cat "$GEN_DIR/mod-update-credentials-folder-level.groovy")" \
--user "$CONTROLLER_TARGET_USER_TOKEN" "${CONTROLLER_TARGET_URL}/scriptText"
...
--user "$CONTROLLER_SOURCE_USER_TOKEN" "${CONTROLLER_SOURCE_URL}/scriptText" -o "$GEN_DIR/tmp-system-credentials.txt"
...
curl -v --data-urlencode "script=$(cat "$GEN_DIR/mod-update-credentials-system-level.groovy")" \
--user "$CONTROLLER_TARGET_USER_TOKEN" "${CONTROLLER_TARGET_URL}/scriptText"

# Reload new Jobs from disk
curl -L -s -u "$CONTROLLER_TARGET_USER_TOKEN" -XPOST "${CONTROLLER_TARGET_URL}/reload"
